...
def zip_first(L1, L2):
    ...
    L3 = list(zip(L1, L2, strict=False))
...
def load_ranking(path, types=None, lazy=False):
    ...
    try:
        ...
        lists = zipstar([li.tolist() for li in tqdm.tqdm(lists)], lazy=lazy)
    except Exception:
        if types is None:
...
def save_ranking(ranking, path):
    ...
    lists = [torch.tensor(li) for li in lists]
...
def load_batch_backgrounds(args, qids):
    ...
    for qid in qids:
        ...
        if len(back) and type(back[0]) is int:
