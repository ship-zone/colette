...
class StridedTensor(StridedTensorCore):
    def __init__(self, packed_tensor, lengths, dim=None, use_gpu=True, gpu_id=None):
        super().__init__(
            packed_tensor, lengths, dim=dim, use_gpu=use_gpu, gpu_id=gpu_id
        )
    ...
    @classmethod
    def pad_packed(cls, packed_tensor, lengths, gpu_id):
        raise AssertionError("This seems to be incorrect but I can't see why. Is it the inner_dims in the views?")

        packed_tensor, lengths = (
            packed_tensor.cuda(gpu_id).contiguous(),
            lengths.cuda(gpu_id),
        )
    ...
    def lookup(self, pids, output="packed"):
        ...
        if self.use_gpu:
            ...
            mask = _create_mask(
                lengths, stride, use_gpu=self.use_gpu, gpu_id=self.gpu_id
            )
    ...
    def _lookup_with_stride(self, stride, lengths, offsets):
        ...
        mask = _create_mask(lengths, stride, use_gpu=self.use_gpu, gpu_id=self.gpu_id)
        # tensor = tensor[mask]
