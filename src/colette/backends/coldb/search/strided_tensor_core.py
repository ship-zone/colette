...
class StridedTensorCore:
    ...
    def as_padded_tensor(self):
        if self.use_gpu:
            view = _create_view(
                self.tensor.cuda(self.gpu_id), self.max_stride, self.inner_dims
            )[self.offsets[:-1]]
            mask = _create_mask(
                self.lengths.cuda(self.gpu_id),
                self.max_stride,
                like=view,
...
def _create_mask(lengths, stride, like=None, use_gpu=True, gpu_id=None):
    if use_gpu:
        mask = torch.arange(stride).cuda(gpu_id) + 1
        mask = mask.unsqueeze(0) <= lengths.cuda(gpu_id).unsqueeze(-1)
    else:
