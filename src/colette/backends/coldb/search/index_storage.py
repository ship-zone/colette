...
class IndexScorer(IndexLoader, CandidateGeneration):
    def __init__(self, index_path, use_gpu=True, load_index_with_mmap=False, gpu_id=None):
        super().__init__(
        ...
        print(f"GUILLAUME indexscorer self.use_gpu {self.use_gpu}")
    ...
    def score_pids(self, config, Q, pids, centroid_scores):
        ...
        if self.use_gpu:
            centroid_scores = centroid_scores.cuda(self.gpu_id)
        ...
        if self.use_gpu:
            ...
            for i in range(0, ceil(len(pids) / batch_size)):
                ...
                pruned_codes_strided = StridedTensor(idx_, codes_lengths, use_gpu=self.use_gpu, gpu_id=self.gpu_id)
                ...
                if approx_scores_.shape[0] == 0:
                    approx_scores.append(torch.zeros((len(pids_),), dtype=approx_scores_.dtype).cuda(self.gpu_id))
                    continue
            ...
            approx_scores_strided = StridedTensor(
                approx_scores, codes_lengths, use_gpu=self.use_gpu, gpu_id=self.gpu_id
            )
            approx_scores_padded, approx_scores_mask = approx_scores_strided.as_padded_tensor()
        ...
        if self.use_gpu:
            D_packed, D_mask = self.lookup_pids(pids, out_device="cuda:" + str(self.gpu_id))
        else:
        ...
        if Q.size(0) == 1:
            return colbert_score_packed(
                Q, D_packed, D_mask, config, self.use_gpu, self.gpu_id, segmented_maxsim=self.segmented_maxsim
            ), pids

        D_strided = StridedTensor(D_packed, D_mask, use_gpu=self.use_gpu, gpu_id=self.gpu_id)
