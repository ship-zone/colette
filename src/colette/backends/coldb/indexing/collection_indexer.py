...
from ..redefines import my_vector_to_array
...
class CollectionIndexer:
    ...
    def setup(self):
        ...
        self.logger.debug(
            f"coldb: sampling {len(sampled_pids)}/{len(self.collection)} docs for index building"
        )
        avg_doclen_est = self._sample_embeddings(sampled_pids)
        self.logger.debug("coldb: avg doclen " + str(avg_doclen_est))
        ...
        self.logger.debug(f"coldb is using {self.num_partitions} partitions")
    ...
    def _sample_pids(self):
        ...
        # typical_doclen = 150  # let's keep sampling independent of the actual doc_maxlen
        if self.num_partitions == -1:
            sampled_pids = 16 * np.sqrt(self.typical_doclen * num_passages)
            # sampled_pids = int(2 ** np.floor(np.log2(1 + sampled_pids)))
        else:
            sampled_pids = int(self.num_partitions * 100 / self.typical_doclen)
    ...
    def _sample_embeddings(self, sampled_pids):
        ...
        if torch.cuda.is_available() and self.use_gpu:
            if torch.distributed.is_available() and torch.distributed.is_initialized():
                ...
                avg_doclen_est = sum(doclens) / len(doclens) if doclens else 0
                avg_doclen_est = torch.tensor([avg_doclen_est]).cuda(self.gpu_id)
    ...
    def _compute_avg_residual(self, centroids, heldout):
        ...
        heldout_reconstruct = compressor.compress_into_codes(
            heldout, out_device=self.dev_str if self.use_gpu else "cpu"
        )
        heldout_reconstruct = compressor.lookup_centroids(
            heldout_reconstruct, out_device=self.dev_str if self.use_gpu else "cpu"
        )
        if self.use_gpu:
            heldout_avg_residual = heldout.cuda(self.gpu_id) - heldout_reconstruct
        else:
...
def compute_faiss_kmeans(
    ...
    use_gpu = False
    faiss.vector_float_to_array = my_vector_to_array
    kmeans = faiss.Kmeans(
        dim, num_partitions, niter=kmeans_niters, gpu=use_gpu, verbose=False, seed=123
    )
