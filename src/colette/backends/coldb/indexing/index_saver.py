...
class IndexSaver:
    ...
    def load_codec(self):
        return ResidualCodec.load(
            index_path=self.config.index_path_, gpu_id=self.gpu_id
        )

    def try_load_codec(self):
        try:
            ResidualCodec.load(index_path=self.config.index_path_, gpu_id=self.gpu_id)
            return True
        except Exception:
            return False
