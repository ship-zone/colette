...
from .residual_embeddings import ResidualEmbeddings
...
class ResidualCodec:
    ...
    def __init__(
        ...
        if self.gpu_id is not None:
            self.dev_str = "cuda:" + str(self.gpu_id)

        ResidualCodec.try_load_torch_extensions(self.use_gpu)

        if self.use_gpu:
            self.centroids = centroids.cuda(self.gpu_id).half()
        else:
        ...
        if torch.is_tensor(self.avg_residual):
            if self.use_gpu:
                self.avg_residual = self.avg_residual.cuda(self.gpu_id).half()

        if torch.is_tensor(bucket_cutoffs):
            if self.use_gpu:
                bucket_cutoffs = bucket_cutoffs.cuda(self.gpu_id)
                bucket_weights = bucket_weights.half().cuda(self.gpu_id)
        ...
        self.arange_bits = torch.arange(
            0,
            self.nbits,
            device=self.dev_str if self.use_gpu else "cpu",
            dtype=torch.uint8,
        )
        ...
        if self.use_gpu:
            self.reversed_bit_map = self.reversed_bit_map.cuda(self.gpu_id)
            if self.decompression_lookup_table is not None:
                self.decompression_lookup_table = self.decompression_lookup_table.cuda(self.gpu_id)
    ...
    def try_load_torch_extensions(cls, use_gpu):
        ...
        print_message("Loading packbits_cpp extension (set COLBERT_LOAD_TORCH_EXTENSION_VERBOSE=True for more info)...")
        packbits_cpp = load(
            name="packbits_cpp",
            sources=[
    ...
    @classmethod
    def load(cls, index_path, gpu_id):
    ...
    def compress(self, embs):
        ...
        for batch in embs.split(1 << 18):
            if self.use_gpu:
                batch = batch.cuda(self.gpu_id).half()
    ...
    def binarize(self, residuals):
        ...
        #         residuals.contiguous().flatten(), self.gpu_id
        #     )
        # else:
        #     residuals_packed = np.packbits(np.asarray(residuals.contiguous().flatten()))
        residuals_packed = np.packbits(np.asarray(residuals.contiguous().cpu().flatten()))
        residuals_packed = torch.as_tensor(residuals_packed, dtype=torch.uint8).cuda(self.gpu_id)
    ...
    def compress_into_codes(self, embs, out_device):
        ...
        for batch in embs.split(bsize):
            if self.use_gpu:
                indices = (self.centroids @ batch.T.cuda(self.gpu_id).half()).max(dim=0).indices.to(device=out_device)
            else:
    ...
    def lookup_centroids(self, codes, out_device):
        ...
        for batch in codes.split(1 << 20):
            if self.use_gpu:
                centroids.append(self.centroids[batch.cuda(self.gpu_id).long()].to(device=out_device))
            else:
    ...
    def decompress(self, compressed_embs: Embeddings):
        ...
        D = []
        for codes_, residuals_ in zip(codes.split(1 << 15), residuals.split(1 << 15), strict=False):
            if self.use_gpu:
                codes_, residuals_ = (
                    codes_.cuda(self.gpu_id),
                    residuals_.cuda(self.gpu_id),
                )
                print(f"residuals_ {residuals_}")
                print(f"self.bucket_weights {self.bucket_weights}")
                print(f"self.reversed_bit_map {self.reversed_bit_map}")
                print(f"self.decompression_lookup_table {self.decompression_lookup_table}")
                print(f"codes_ {codes_}")
                print(f"self.centroids {self.centroids}")
                print(f"self.dim {self.dim}")
                print(f"self.nbits {self.nbits}")
