...
from . import residual_embeddings
...
class ResidualEmbeddingsStrided:
    def __init__(self, codec, embeddings, doclens):
        ...
        self.gpu_id = self.codec.gpu_id

        self.codes_strided = StridedTensor(
            self.codes, doclens, use_gpu=self.use_gpu, gpu_id=self.gpu_id
        )
        self.residuals_strided = StridedTensor(
            self.residuals, doclens, use_gpu=self.use_gpu, gpu_id=self.gpu_id
        )
