...
class ColBERT(BaseColBERT):
    ...
    def __init__(self, name="bert-base-uncased", colbert_config=None, gpu_id=None):
        super().__init__(name, colbert_config, gpu_id)
...
def colbert_score(Q, D_padded, D_mask, config=None, use_gpu=None, gpu_id=None, seg_maxsim=None):
    """
    ...
    if use_gpu:
        Q, D_padded, D_mask = Q.cuda(gpu_id), D_padded.cuda(gpu_id), D_mask.cuda(gpu_id)
...
def colbert_score_packed(Q, D_packed, D_lengths, config=None, use_gpu=None, gpu_id=None, segmented_maxsim=None):
    """
    ...
    if use_gpu:
        Q, D_packed, D_lengths = (
            Q.cuda(gpu_id),
            D_packed.cuda(gpu_id),
            D_lengths.cuda(gpu_id),
        )
    ...
    if use_gpu or config.interaction == "flipr":
        scores_padded, scores_mask = StridedTensor(scores, D_lengths, use_gpu=use_gpu, gpu_id=gpu_id).as_padded_tensor()

        return colbert_score_reduce(scores_padded, scores_mask, config)
    else:
        return segmented_maxsim(scores, D_lengths)
