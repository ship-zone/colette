...
from ..tokenization.utils import _split_into_batches
...
class QueryTokenizer:
    ...
    def tensorize(
        ...
        device_ = (
            torch.device("cpu")
            if gpu_id is None
            else torch.device("cuda:" + str(gpu_id))
        )
        ...
        assert not full_length_search or (
            type(batch_text) is list and len(batch_text) == 1
        )
