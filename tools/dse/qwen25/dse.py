...
class DSEModel(nn.Module):
    ...
    def encode_query(self, qry):
        ...
        #     0, qry["input_ids"].shape[0], device=qry["input_ids"].device
        # )
        # cache_position = None
        # qry = self.encoder.prepare_inputs_for_generation(
        #     **qry, use_cache=False, cache_position=cache_position
        # )
    ...
    def encode_passage(self, psg):
        # cache_position = torch.arange(
        #     0, psg["input_ids"].shape[0], device=psg["input_ids"].device
        # )
        # cache_position = None
        # psg = self.encoder.prepare_inputs_for_generation(
        #     **psg, use_cache=False, cache_position=cache_position
        # )
